...
def send_question(message):
    ...
    usage_stats['daily'] += 1
    usage_stats['weekly'] += 1
    usage_stats['monthly'] += 1
...
def activite(message):
 if str(message.from_user.id) in admins:
    ...
    weekly_usage = usage_stats['weekly']
    monthly_usage = usage_stats['monthly']
    ...
    bot.send_message(message.chat.id, f"Статистика использования бота:\n"
                                     f"За сутки: {daily_usage}\n"
                                     f"За неделю: {weekly_usage}\n"
                                     f"За месяц: {monthly_usage}\n"
